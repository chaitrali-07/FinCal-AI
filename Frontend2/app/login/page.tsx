...
// Google SVG icon (avoid Chrome icon confusion)
function GoogleIcon() {
  return (
...
function getFriendlyError(error: unknown): string {
  ...
  if (msg.includes("auth/popup-closed-by-user") || msg.includes("auth/cancelled-popup-request"))
    return ""; // User closed popup intentionally — don't show error
  ...
  if (msg.includes("auth/unauthorized-domain"))
    return "This domain isn't authorized. Add 'localhost' in Firebase Console → Authentication → Settings → Authorized domains.";
  if (msg.includes("auth/internal-error"))
    return "An internal error occurred. Make sure Google sign-in is enabled in Firebase Console.";
  return msg.replace("Firebase: ", "").replace(/\s*\(auth\/[^)]+\)\.?/, "").trim() || "Sign-in failed. Please try again.";
}
...
export default function LoginPage() {
  ...
  // true while we check if user is returning from a Google redirect
  const [checkingRedirect, setCheckingRedirect] = useState(true);

  // On mount: check if returning from signInWithRedirect
  useEffect(() => {
    getRedirectResult(auth)
      .then((result) => {
        if (result?.user) {
          // Redirect sign-in succeeded — navigate to app
  ...
  // If already signed in (session persisted), redirect
  useEffect(() => {
    if (!authLoading && user) {
      router.replace("/dashboard");
    }
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center">
          ...
          <p className="text-muted-foreground text-sm mt-1">Sign in to your Finova account</p>
        </div>
